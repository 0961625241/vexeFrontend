

//Signup
export const LIST_USER="LIST_USER";
export const POST_SIGNUP_REQUEST="POST_SIGNUP_REQUEST";
export const POST_LOGIN_REQUEST="POST_LOGIN_REQUEST";
export const GET_USER_ID = "GET_USER_ID";
export const POST_LOGIN_FAILED = "POST_LOGIN_FAILED";
export const POST_LOGIN_SUCCESS = "POST_LOGIN_SUCCESS";


// admin delete update 
export const DELETE_USER = "DELETE_USER";
export const UPDATE_USER = "UPDATE_USER"


export const LIST_STATION="LIST_STATION";
export const ADD_STATION="ADD_STATION";
export const UPDATE_STATION="UPDATE_STATION";
export const DELETE_STATION="DELETE_STATION";

export const LIST_TRIP="LIST_TRIP";
export const ADD_TRIP="ADD_TRIP";
export const UPDATE_TRIP="UPDATE_TRIP";
export const DELETE_TRIP="DELETE_TRIP";
export const GET_ID_TRIP="GET_ID_TRIP";




export const LIST_TICKET="LIST_TICKET";
export const DELETE_TICKET="DELETE_TICKET";

export const LIST_CAR  = "LIST_CAR";
export const ADD_CAR  = "ADD_CAR";
export const UPDATE_CAR  = "UPDATE_CAR";
export const DELETE_CAR  = "DELETE_CAR";


export const LIST_DRIVER  = "LIST_DRIVER";
export const ADD_DRIVER  = "ADD_DRIVER";
export const UPDATE_DRIVER  = "UPDATE_DRIVER";
export const DELETE_DRIVER  = "DELETE_DRIVER";

export const LIST_PROVINCE  = "LIST_PROVINCE";


export const LIST_CARMFG  = "LIST_CARMFG";


export const LIST_BUSA1  = "LIST_BUSA1";
export const ADD_BUSA1  = "ADD_BUSA1";
export const UPDATE_BUSA1  = "UPDATE_BUSA1";
export const DELETE_BUSA1  = "DELETE_BUSA1";

export const SELECT="SELECT";


export const FILTER_SEAT="FILTER_SEAT";
export const SORT_PRICE="SORT_PRICE";
export const SEARCH="SEARCH";
export const SEARCH_DRIVER="SEARCH_DRIVER";
export const SEARCH_BUSA1="SEARCH_BUSA1";
export const SEARCH_TRIP="SEARCH_TRIP";
export const SEARCH_CAR="SEARCH_CAR";
export const SEARCH_USER="SEARCH_USER";




