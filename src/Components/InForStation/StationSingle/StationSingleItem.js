...
class StationSingleItem extends Component {
    constructor(props) {
        super(props)
    ...
    render() {
        ...
        return (
            <>
                <tr>
                    <td >{item.fromStation.nameStation} đi {item.toStation.province.nameProvince}</td>
                    <td>{item.cars.CarMFG.nameCarMFG}</td>
                    <td>{item.price}</td>
                    <td >{new Date(item.startTime).toLocaleDateString("es-CL")}
...
export default connect(mapStateToProps, mapDispathToProps)(StationSingleItem);